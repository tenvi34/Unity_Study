Shader "EGA/Particles/FireSphere"
{
    ...
    SubShader
    {
        Tags
        {
            "RenderType" = "Transparent" "Queue" = "Transparent+0" "IgnoreProjector" = "True" "IsEmissive" = "true" "PreviewType"="Plane"
        }
        Cull Back
        CGPROGRAM
        ...
        void surf(Input i, inout SurfaceOutput o)
        {
            ...
            float2 temp_output_8_0 = (((float2(0.2, 0) * _Time.y) + (i.
                    uv_tex3coord).xy + i.uv_tex3coord.z) *
                _StartFrequency);
            ...
            float2 break17 = (temp_output_10_0 * temp_output_10_0 * (temp_cast_1
                - (temp_output_10_0 * 2.0)));
            float lerpResult39 = lerp(frac((473.5 * sin(temp_output_21_0))),
                frac((473.5 * sin((1.0 + temp_output_21_0)))), break17.x);
            float lerpResult38 = lerp(
                frac((473.5 * sin((57.0 + temp_output_21_0)))),
                frac((473.5 * sin((58.0 + temp_output_21_0)))),
                break17.x);
            float lerpResult40 = lerp(lerpResult39, lerpResult38, break17.y);
            float3 temp_output_51_0 = ((float3((float2(0.5, 0.5) * _Time.y),
                0.0) + (i.uv_tex3coord * (lerpResult40 *
                _Amplitude)) + i.uv_tex3coord.z) * _Frequency);
            ...
            float3 break110 = (temp_output_52_0 * temp_output_52_0 * (
                temp_cast_3 - (temp_output_52_0 * 2.0)));
            float lerpResult109 = lerp(frac((473.5 * sin(temp_output_90_0))),
                frac((473.5 * sin((1.0 + temp_output_90_0)))), break110.x);
            float lerpResult105 = lerp(
                frac((473.5 * sin((57.0 + temp_output_90_0)))),
                frac((473.5 * sin((58.0 + temp_output_90_0)))), break110.x);
            float lerpResult106 =
                lerp(lerpResult109, lerpResult105, break110.y);
            float Amp114 = _Amplitude;
            float4 tex2DNode117 = tex2D(_MainTex,
                   (i.uv_tex3coord + (0.2 * (lerpResult106 * Amp114))).xy);
            o.Emission = lerp(temp_output_121_0,
                (temp_output_121_0 * tex2DNode117), _Useblack).rgb;
            float4 clampResult132 = clamp(
                (i.vertexColor.a * tex2DNode117 * _Opacity), float4(0, 0, 0, 0),
                float4(1, 1, 1, 1));
            float4 ase_screenPos = float4(i.screenPos.xyz,
                i.screenPos.w + 0.00000000001);
            float4 ase_screenPosNorm = ase_screenPos / ase_screenPos.w;
            ase_screenPosNorm.z = (UNITY_NEAR_CLIP_VALUE >= 0)
                ? ase_screenPosNorm.z
                : ase_screenPosNorm.z * 0.5 + 0.5;
            float screenDepth137 = LinearEyeDepth(
                SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture,
                    ase_screenPosNorm.xy));
            float distanceDepth137 = abs(
                (screenDepth137 - LinearEyeDepth(ase_screenPosNorm.z)) / (
                    _Depthpower));
            float clampResult136 = clamp(distanceDepth137, 0.0, 1.0);
            o.Alpha = lerp(clampResult132, (clampResult132 * clampResult136),
   _Usedepth).r;
        }
        ENDCG
    }
